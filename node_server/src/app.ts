...
io.on("connection", (socket) => {
  ...
  socket.on("start_game", async function(roomId: number) {
    socket.to(roomId.toString()).emit("game_started", JSON.stringify({ data: {player: "Aquib"}, action: "game_started" }));
    await Room.findOneAndUpdate(
      {roomId},
      {status: GameStatus.IN_PROGRESS}
    );  
    const cards = divideAndShuffleCards();
    // This is a specific room
    ...
    Object.keys(socketRoom).map((key: string, index) => {
      if (key === "sockets") {  
        Object.keys(socketRoom[key]).map((socketId: string) => {
          ...
          io.to(socketId).emit("opening_hand", { data: {cards: cards}, action: "opening_hand" });
        });
      }
    });
    // Send to a specific player
    socket.to(roomId.toString()).emit("opening_hand", JSON.stringify({ data: { cards: cards }, action: "opening_hand" }));
  });
